---
params:
  project_name: BiB_AgeOfWonder
---





# BiB Age of Wonder {#bib_ageofwonder}


```{r echo=FALSE}
dl_types <- c('Table detail', 'Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder
_tables.csv', '
bib_ageofwonder
variables_meta.csv', '
bib_ageofwonder
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



BiB Age of Wonder surveys, health measures and associated data



```{r, include=FALSE}
prj_name <- 'BiB_AgeOfWonder'
source('../R/get_all_project_metadata.R', local = knitr::knit_global())
```



```{r, include=FALSE}
source('../R/make_project_tables_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_project_tables.R'}

```





## AoW recruitment denominator 2023 release {#bib_ageofwonder_recruitment_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_recruitment_dr23
_variables_meta.csv', '
bib_ageofwonder_recruitment_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.1.0) full recruitment denominator. Contains records for all consented participants in the period of the data release, regardless of data collected. Includes ages, dates, school id and form tutor id plus some other contextual indicators (free school meals, SEN) and IMD (2019). Also has data availability indicators for each participant. <br> Each record contains a unique recruitment event, with a new recuitment event occuring at each new school year, and at each new school. Therefore, it's possible that participants will be listed multiple times in this table. aow_recruitment_id is unique at each record, whilst aow_person_id can be used to identify unique participants in AoW. BiBPersonID can be used to link BiB participants in AoW to other BiB study data.



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.recruitment_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```





## AoW survey module 1 2023 release {#bib_ageofwonder_survey_mod01_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_survey_mod01_dr23
_variables_meta.csv', '
bib_ageofwonder_survey_mod01_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.0.0) student survey module 1



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.survey_mod01_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```





## AoW survey module 2 2023 release {#bib_ageofwonder_survey_mod02_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_survey_mod02_dr23
_variables_meta.csv', '
bib_ageofwonder_survey_mod02_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.0.0) student survey module 2



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.survey_mod02_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```





## AoW survey module 3 2023 release {#bib_ageofwonder_survey_mod03_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_survey_mod03_dr23
_variables_meta.csv', '
bib_ageofwonder_survey_mod03_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.0.0) student survey module 3



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.survey_mod03_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```





## AoW survey module 4 2023 release {#bib_ageofwonder_survey_mod04_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_survey_mod04_dr23
_variables_meta.csv', '
bib_ageofwonder_survey_mod04_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.0.0) student survey module 4



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.survey_mod04_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```





## AoW school visit height and weight 2023 release {#bib_ageofwonder_heightweight_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_heightweight_dr23
_variables_meta.csv', '
bib_ageofwonder_heightweight_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.0.0) school visit height and weight measures.



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.heightweight_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```





## AoW school visit bioimpedance 2023 release {#bib_ageofwonder_bioimpedance_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_bioimpedance_dr23
_variables_meta.csv', '
bib_ageofwonder_bioimpedance_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.0.0) school visit bioimpedance data.



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.bioimpedance_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```





## AoW school visit skinfold 2023 release {#bib_ageofwonder_skinfold_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_skinfold_dr23
_variables_meta.csv', '
bib_ageofwonder_skinfold_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.0.0) school visit skinfold measures.



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.skinfold_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```





## AoW school visit blood pressure 2023 release {#bib_ageofwonder_bloodpressure_dr23}


```{r echo=FALSE}
dl_types <- c('Variable detail', 'Variable names')
dl_csvs <- c('
bib_ageofwonder_bloodpressure_dr23
_variables_meta.csv', '
bib_ageofwonder_bloodpressure_dr23
_variables_names.csv')
make_download_table(dl_types, dl_csvs)
```



[Annual data release 2023](https://github.com/BornInBradford/aow_datarelease/releases/tag/v2023.0.0) school visit blood pressure data.



```{r, include=FALSE}
tab_id <- 'BiB_AgeOfWonder.bloodpressure_dr23'
prj_name <- 'BiB_AgeOfWonder'
source('../R/make_table_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_table_info.R'}

```



```{r, include=FALSE}
source('../R/make_var_info_display.R', local = knitr::knit_global())
```



```{r, echo=FALSE, file='../R/reactable_vars.R'}

```



